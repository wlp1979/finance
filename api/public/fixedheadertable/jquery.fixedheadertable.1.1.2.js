...
(function($)
{
	...
	function buildTable(obj,options) {
		...
		if(hasTable && hasTHead && hasTBody) {
			...
			obj.find('div.fht_fixed_footer').remove();
			...
			if (options.footer && !options.cloneHeaderToFooter) {
			    var footerHeight = table.find('tfoot').outerHeight();
			
				obj.find('div.fht_table_body table tfoot tr:first th').each(function() {
			...
			if (options.footer && !options.cloneHeaderToFooter) {
	            if (obj.find('div.fht_fixed_footer').size() == 0) {
	                obj.append('<div class="fht_fixed_footer"><table width="'+obj.find('div.fht_table_body table').width()+'px"></table></div>');
	            }
	            obj.find('div.fht_fixed_footer').css({'width':parentDivWidth + 'px', 'height':table.find('tfoot').outerHeight() + 1 + 'px'});
            ...
            tableHeader.clone(true).appendTo('div.fht_fixed_header table');

            obj.find('div.fht_table_body table').css({'margin-top':-(tableHeader.height())+'px'});
            
            if (options.footer) {
				if(options.cloneHeaderToFooter) {
	            	// put cloned header here
	            	obj.find('div.fht_fixed_header').clone().appendTo(obj);
				}
				else {
					var tableFooter = obj.find('div.fht_table_body table tfoot');
					tableFooter.clone(true).appendTo('div.fht_fixed_footer table');
					tableFooter.remove();
		            // obj.find('div.fht_table_body table').css({'margin-bottom':-(tableFooter.height())+'px'});
				}
            }
